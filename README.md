# Installation

## Using Map Manager

- Find the latest release on the [Releases page](https://github.com/devenperez/subway-builder-canadian-maps/releases)

  > WARNING: Saves on map version 1.0.1 or earlier are NOT compatible with newer versions! The city codes were changed to IATA codes for future compatibility with other maps. You can continue playing those saves with [map version 1.0.1](https://github.com/devenperez/subway-builder-canadian-maps/releases/tag/v1.0.1)

- Download the zip file for the city (or cities) you would like to play

- Use Subway Builder Map Manager to add maps
    - For detailed instructions on this process, follow [this guide](https://subwaybuildermodded.com/wiki/maps/map-installation-guide)
 
## Using Railyard

**COMING SOON**
